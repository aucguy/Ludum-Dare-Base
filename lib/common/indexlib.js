...
function renderLoadingScreen() {
  ...
  context.stroke();
  context.beginPath();
  var progress = loading === 0 ? 0 : loaded / loading;
  context.rect(20, height - 40, progress * (width - 40), 20);
  context.fill();
  
  requestAnimationFrame(renderLoadingScreen);
};

document.addEventListener('DOMContentLoaded', renderLoadingScreen);
...
function loadScript(name, location, type) {
  return new Promise((resolve, reject) => {
    console.log(`adding ${type}: ${location}`);
...
function loadManifest(url, base) {
  ...
  return fetch(url).then(response => {
    ...
    for(var item of manifest.items) {
      let name = item[0];
      let location = join(base, item[1]);
      let type = item[2];
      
      promise = promise.then(() => loadAsset(name, location, type));
    }
